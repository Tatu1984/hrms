...
export default function TeamAttendanceCalendar({ employees }: { employees: Employee[] }) {
  ...
  useEffect(() => {
    ...
    const fetchMonthAttendance = async () => {
      ...
      try {
        const response = await apiFetch(`/api/attendance?startDate=${startDate}&endDate=${endDate}`);
  ...
  const handleEmployeeClick = async (employee: Employee, attendanceRecord: AttendanceRecord) => {
    ...
    try {
      // Fetch activity logs
      const activityResponse = await apiFetch(`/api/attendance/activity?attendanceId=${attendanceRecord.id}`);
      ...
      const updateResponse = await apiFetch(`/api/daily-work-updates?employeeId=${employee.id}&date=${dateStr}`);
