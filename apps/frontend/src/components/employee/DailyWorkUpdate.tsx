...
export default function DailyWorkUpdate() {
  ...
  useEffect(() => {
    const fetchMonthUpdates = async () => {
      ...
      try {
        const response = await apiFetch(`/api/daily-work-updates?month=${month}`);
        if (response.ok) {
          const data = await response.json();
          const dates = new Set(
  ...
  useEffect(() => {
    const fetchUpdateForDate = async () => {
      ...
      try {
        const response = await apiFetch(`/api/daily-work-updates?date=${dateStr}`);
        if (response.ok) {
          const data = await response.json();
  ...
  const handleSave = async () => {
    ...
    try {
      const response = await apiFetch('/api/daily-work-updates', {
        method: 'POST',
        headers: {
  ...
  const handleDelete = async () => {
    ...
    try {
      const response = await apiFetch(`/api/daily-work-updates?id=${currentUpdate.id}`, {
        method: 'DELETE',
      });
